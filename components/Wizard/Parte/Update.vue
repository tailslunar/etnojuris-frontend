<template>
    <v-dialog width="600" @input="iniciar">
        <template v-slot:activator="{ on, attrs }">
            <v-btn text color="roxo" v-bind="attrs" v-on="on"><v-icon>mdi-pencil</v-icon></v-btn>
        </template>
...
export default {
    ...
    computed: {
        objParte() {
            ...
            let parte = {
                id, papel, categoria, nome, cpf_cnpj
            }
    ...
    methods: {
        ...mapActions('processo', ['updateParteId']),
        iniciar(val) {
            if (val) {
                this.iniciarParte()
        ...
        iniciarParte() {
            this.field.nome = this.parte.nome
            this.field.cpf_cnpj = this.parte.cpf_cnpj
            this.field.papel = this.parte.papel
            this.field.categoria = this.parte.categoria
            let categoria = this.field.categoria
            if (categoria == 'advogado') {
                this.field[categoria].nome = this.parte[categoria].nome
                this.field[categoria].oab = this.parte[categoria].oab
                this.field[categoria].uf = this.parte[categoria].uf
                this.field[categoria].instituicao = this.parte[categoria].instituicao
            }
            if (categoria == 'procurador') {
                this.field[categoria].instituicao = this.parte[categoria].instituicao
            }
            if (categoria == 'defensoria') {
                this.field[categoria].instituicao = this.parte[categoria].instituicao
            }
        },
        ...
        submitParte(dialog) {
            dialog.value = false
            this.updateParteId(this.objParte)
