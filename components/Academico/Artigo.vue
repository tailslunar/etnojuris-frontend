<template>
  <div class="item-repositorio">
    ...
    <section><strong>Categoria:</strong> {{ display.categoria }}</section>
    <section><strong>Autoria:</strong> {{ display.autoria }}</section>
    <section><strong>Ano:</strong> {{ display.ano }}</section>
    <section><strong>Referência da publicação:</strong> {{ display.referencia }}</section>
  </div>
</template>
...
export default {
  ...
  computed: {
    ...
    display() {
      return {
        id: this.artigo.id,
        titulo: this.artigo.academico_titulo || '',
        categoria: this.artigo.academico_tipo || '',
        autoria: this.parseAutores(this.artigo.academico_autor),
        ano: this.artigo.academico_ano || '',
        referencia: this.artigo.academico_ano || '',
      }
    }
  },
  methods: {
    ...
    abrirArtigo() {
      let { id, academico_titulo } = this.artigo;
      if (this.to) {
        let urlMagica = academico_titulo
        ? academico_titulo
        ...
        this.$router.push({
          name: 'dashboard-academico-artigo',
          params: { artigo: id }
        });
      }
    },
    ...
    parseAutores(autoresJson) {
        try {
            const autoresArray = JSON.parse(autoresJson);
            return autoresArray.map(autorObj => autorObj.autor).join(', ');
        } catch (error) {
