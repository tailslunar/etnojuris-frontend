<template>
  <section>
    <CheckboxSvgMap :map="BrazilMap" v-model="selecionados" @mouseover="atualizaProcessoQuilombo"
      @mousemove="updatePosicaoMouse" @mouseleave="mouse.state = false" />
    <div v-if="mouse.state" class="state-name-box" :style="{ top: mouse.y + 'px', left: mouse.x + 'px' }">
      <h1>{{ mapEstado.estado }}</h1>
      <div>Processos: {{ mapEstado.processo }}</div>
      <div>Quilombos: {{ mapEstado.quilombo }}</div>
    </div>
    <div style="display: flex; gap: 10px; flex-wrap: wrap">
      <v-btn v-for="regiao in Object.keys(dashboard.regiao)" :key="regiao" class="botao" color="roxo"
        :outlined="!dashboard.regiao[regiao].selected" x-small @click.prevent="regiaoIs(regiao)">
...
export default {
  ...
  methods: {
    ...
    regiaoIs(regiao) {
      // Verifica se a região já está selecionada
      const regiaoAtiva = this.dashboard.regiao[regiao].selected;

      if (regiaoAtiva) {
        // Se a região já estiver selecionada, resetar para todos os estados do Brasil
        this.setDashboardEstado(['todos']);
        this.resetRegiaoSelected()
      } else {
        // Caso contrário, selecione os estados dessa região
        this.setDashboardRegiaoSelected(regiao); // Chama a mutação que seleciona a região
        this.selecionados = this.dashboard.regiao[regiao].estados; // Atualiza os selecionados com os estados da região
      }
    },
