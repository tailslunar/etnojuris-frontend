...
export default {
    ...
    methods: {
        ...
        async iniciarQuilombo() {
            ...
            try {
                const { data: { data } } = await this.$axios.get(`/api/tb_quilombo/${this.id}`);
                this.quilombo = {
                    nome: data.nome ?? '',
                ...
                this.mapa.center = [data.latitude, data.longitude]
                await this.buscarCEP(false)
             } catch (error) {
                console.error(error);
            }
        },
        async submitLocalidadeQuilombo() {
            ...
            try {
                const localidadeExistente = this.localidades.find(loc => loc.uf === this.localidade.uf && loc.cidade === this.localidade.cidade);
                if (localidadeExistente) {
                    this.localidade.id = localidadeExistente.id;
                } else {
                    const novaLocalidade = await this.submitLocalidade();
                    this.localidade.id = novaLocalidade.id;
                }
        ...
        async submitQuilombo() {
            try {
                const response = await this.$axios.put(`/api/tb_quilombo/${this.id}`, this.objQuilombo);
