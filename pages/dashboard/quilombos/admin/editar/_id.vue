<template>
    <Canvas header_with_template="Editar quilombo" :short="true" v-if="!!isAdmin">
        ...
        <v-form v-model="isValid" autocomplete="false">
            <div class="formQuilombo_info">
                ...
                <v-text-field label="CNPJ" outlined v-model="quilombo.cnpj"
                    v-mask='"##.###.###/####-##"'></v-text-field>
            </div>
            ...
            <div class="formQuilombo_localidade">
                ...
                <v-select label="Estado" outlined :items="localidadeEstado" v-model="localidade.uf"
                    :rules="rules.estado"></v-select>
                <v-select label="Cidade" outlined :items="localidadeCidade" v-model="localidade.id"
                    :rules="rules.cidade"></v-select>
                <div>
...
export default {
    ...
    watch: {
        'quilombo.localidade_id'(v) {            
            let loc = this.localidades.find(l => l.id == v)
            console.log('QUILOMBO CARREGADO', v, loc)
            if(loc){
                this.localidade.id = loc.id
                this.localidade.uf = loc.uf
                this.localidade.cidade = loc.cidade
            }
        },
        'localidade.id'(v) {
            let loc = this.localidades.find(l => l.id == v)
    ...
    methods: {
        ...
        async iniciarQuilombo() {
            let { id } = this.$route.params
            try {
                const { data: { data } } = await this.$axios.get(`/api/tb_quilombo/${id}`);
                this.quilombo = { ...this.quilombo, ...data }
            } catch (error) {
                console.error(error);
            }
        },
        ...
        async submitQuilombo() {
            let { id } = this.$route.params
            try {
                const response = await this.$axios.put(`/api/tb_quilombo/${id}`, this.objQuilombo);
