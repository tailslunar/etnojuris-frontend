    ...
    methods: {
        ...
        async iniciarRepositorio() {
            ...
            try {
                ...
                this.dataRepositorio = data

                this.repositorio.tipo_publicacao = data.tipo_publicacao;

                this.repositorio.noticia = Object.fromEntries(Object.entries(data).filter(([key]) => key.startsWith('noticia_')))
                this.repositorio.jurisprudencia = Object.fromEntries(Object.entries(data).filter(([key]) => key.startsWith('jurisprudencia_')))
                this.repositorio.legislacao = Object.fromEntries(Object.entries(data).filter(([key]) => key.startsWith('legislacao_')))
                this.repositorio.academico = Object.fromEntries(Object.entries(data).filter(([key]) => key.startsWith('academico_')))

                this.autores = JSON.parse(data.academico_autor).map(a => ({ id: uuidv4(), lattes:null,  ...a }))
