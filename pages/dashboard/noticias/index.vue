    ...
    <template slot="coluna">
      <div class="rightIsAdmin">
        <v-btn depressed class="botao" color="amarelo_mostarda"
          :to="{ name: 'dashboard-repositorio-admin-novo', query: { tipo: 'noticia' } }">
    ...
    <div class="repositorio">
      <header>{{ noticiasComFiltro.length }} notícias
        <v-btn v-if="hasQuery" text class="botao limpar" color="amarelo_mostarda"
          :to="{ name: 'dashboard-noticias' }"><v-icon>mdi-close</v-icon> Limpar pesquisa </v-btn>
...
export default {
  ...
  methods: {
    ...
    irParaPagina(pagina) {
      ...
      this.$router.push({ name: "dashboard-noticias", query: { ...query, pagina } });
    }
  }
}
</script>
